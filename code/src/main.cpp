...
int main(int argc, char** argv) {
	...
	// Init scene here
	///////////////////////////////////////////////
	Renderer *renderer = new ExSimpleCube(display_w, display_h);
	///////////////////////////////////////////////
	...
	while (!quit_app) {
		...
		while (SDL_PollEvent(&eve)) {
			...
			case SDL_WINDOWEVENT:
				if (eve.window.event == SDL_WINDOWEVENT_RESIZED) {
					renderer->GLResize(eve.window.data1, eve.window.data2);
				}
				break;
			case SDL_QUIT:
				quit_app = true;
		...
		if(!io.WantCaptureMouse) {
			...
			renderer->GLmousecb(ev);
		}
		renderer->GLrender((float)expected_frametime);
